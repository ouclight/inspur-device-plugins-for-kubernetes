...
//light
var (
	patchLog = ctrl.Log.WithName("patch")
)
//end
...
func (p *patcher) AddAf(accfunc *fpgav2.AcceleratorFunction) error {
	...
	p.Lock()
//light
patchLog.Info("Stepped into AddAf | namespace: %s, accfunc.Name:%s",namespace,accfunc.Name);
//end
	...
	} else if accfunc.Spec.Mode == bsp { 
		devtype := fmt.Sprintf("%s-%s", "f10a", accfunc.Spec.InterfaceID)
		p.resourceMap[namespace+"/"+accfunc.Name] = rfc6901Escaper.Replace(namespace + "/" + devtype)
	} else {
...
//light
patchLog.Info("rname:%s,mappedName:%s\n",rname,mappedName);
//end
